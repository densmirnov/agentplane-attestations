...
function validateArtifactPayload(artifact, path, errors) {
  ...
  switch (artifact.kind) {
    case 'intent':
      ...
      break;
    case 'approval':
      if (!['approved', 'rejected', 'waived', 'missing'].includes(payload.status)) {
        errors.push(`${path}.payload.status must be approved, rejected, waived, or missing.`);
      }
      break;
    case 'execution':
      ...
      if (payload.filesChanged !== undefined && !Array.isArray(payload.filesChanged)) {
        errors.push(`${path}.payload.filesChanged must be an array when present.`);
      }
      break;
    case 'verification':
    ...
    case 'anchor':
      ...
      if (!isNonEmptyString(payload.txUrl) && !isNonEmptyString(payload.reference)) {
...
function validateArtifact(artifact, index, errors) {
  ...
  if (!ARTIFACT_KINDS.has(artifact.kind)) {
    errors.push(`${path}.kind must be one of ${Array.from(ARTIFACT_KINDS).join(', ')}.`);
  }
...
function summarizeArtifact(artifact) {
  return {
    ...
    payloadDigest: sha256Hex(canonicalStringify(canonicalize(artifact.payload))),
...
function normalizeExecution(bundle, executionArtifact) {
  ...
  return {
    ...
    commit: executionArtifact.payload.commit ?? bundle.context.source.commit ?? null,
...
export function isArtifactBundleInput(input) {
  return isObject(input) && Array.isArray(input.artifacts) && isNonEmptyString(input.bundleId);
}
...
export function validateArtifactBundle(bundle) {
    ...
    for (const [index, artifact] of bundle.artifacts.entries()) {
      ...
      if (isNonEmptyString(artifact?.artifactId)) {
        if (seenArtifactIds.has(artifact.artifactId)) {
          errors.push(`artifacts[${index}].artifactId must be unique within the bundle.`);
        }
  ...
  if (!Array.isArray(bundle.artifacts) || !bundle.artifacts.some((artifact) => artifact.kind === 'intent')) {
...
export function normalizeArtifactBundle(inputBundle) {
  ...
  const noteArtifacts = bundle.artifacts.filter((artifact) => artifact.kind === 'note');
  ...
  const evidence = {
    meta: {
      ...
      maintainer: bundle.subject.operator?.displayName ?? bundle.subject.operator?.id ?? null
    },
    ...
    task: {
      ...
      approvedBy: approvalArtifact?.producer.displayName ?? approvalArtifact?.producer.id ?? null,
    ...
    approvals: {
      human: {
        ...
        actor: approvalArtifact?.producer.displayName ?? approvalArtifact?.producer.id ?? null
      }
    },
    anchors: {
      registration: anchorArtifact
        ? {
            ...
            txUrl: anchorArtifact.payload.txUrl ?? anchorArtifact.payload.reference ?? null
          }
        : null
    },
