...
function makeClaims(evidence) {
  ...
  return {
    ...
    executionObserved: Boolean(evidence?.execution?.repo && evidence?.execution?.branch),
    verificationPassed:
...
export function createAttestation(inputEvidence) {
  const evidence = canonicalize(inputEvidence);
...
export function verifyAttestation(attestation) {
  ...
  const expectedEvidenceDigest = sha256Hex(canonicalStringify(canonicalize(attestation.evidence)));
